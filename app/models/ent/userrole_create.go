...
func (urc *UserRoleCreate) SetDescription(s string) *UserRoleCreate {
...
func (urc *UserRoleCreate) AddOwnerIDs(ids ...int) *UserRoleCreate {
	urc.mutation.AddOwnerIDs(ids...)
	return urc
}

// AddOwner adds the "owner" edges to the User entity.
func (urc *UserRoleCreate) AddOwner(u ...*User) *UserRoleCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return urc.AddOwnerIDs(ids...)
}

// AddPermissionIDs adds the "permissions" edge to the RolePermission entity by IDs.
func (urc *UserRoleCreate) AddPermissionIDs(ids ...int) *UserRoleCreate {
	urc.mutation.AddPermissionIDs(ids...)
	return urc
}

// AddPermissions adds the "permissions" edges to the RolePermission entity.
func (urc *UserRoleCreate) AddPermissions(r ...*RolePermission) *UserRoleCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return urc.AddPermissionIDs(ids...)
}
...
func (urc *UserRoleCreate) createSpec() (*UserRole, *sqlgraph.CreateSpec) {
	...
	if nodes := urc.mutation.OwnerIDs(); len(nodes) > 0 {
	...
	if nodes := urc.mutation.PermissionsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
