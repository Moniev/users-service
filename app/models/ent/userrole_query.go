...
func (urq *UserRoleQuery) QueryOwner() *UserQuery {
...
func (urq *UserRoleQuery) QueryPermissions() *RolePermissionQuery {
	query := (&RolePermissionClient{config: urq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := urq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (urq *UserRoleQuery) Clone() *UserRoleQuery {
	...
	return &UserRoleQuery{
		...
		withOwner:       urq.withOwner.Clone(),
		withPermissions: urq.withPermissions.Clone(),
...
func (urq *UserRoleQuery) WithOwner(opts ...func(*UserQuery)) *UserRoleQuery {
	query := (&UserClient{config: urq.config}).Query()
...
func (urq *UserRoleQuery) WithPermissions(opts ...func(*RolePermissionQuery)) *UserRoleQuery {
	query := (&RolePermissionClient{config: urq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	urq.withPermissions = query
	return urq
}
...
func (urq *UserRoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserRole, error) {
	...
	if query := urq.withOwner; query != nil {
		if err := urq.loadOwner(ctx, query, nodes,
			func(n *UserRole) { n.Edges.Owner = []*User{} },
			func(n *UserRole, e *User) { n.Edges.Owner = append(n.Edges.Owner, e) }); err != nil {
			return nil, err
		}
	}
	if query := urq.withPermissions; query != nil {
		if err := urq.loadPermissions(ctx, query, nodes,
			func(n *UserRole) { n.Edges.Permissions = []*RolePermission{} },
			func(n *UserRole, e *RolePermission) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (urq *UserRoleQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(userrole.OwnerTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(userrole.OwnerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(userrole.OwnerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(userrole.OwnerPrimaryKey[1]))
...
func (urq *UserRoleQuery) loadPermissions(ctx context.Context, query *RolePermissionQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *RolePermission)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*UserRole)
	nids := make(map[int]map[*UserRole]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(userrole.PermissionsTable)
		s.Join(joinT).On(s.C(rolepermission.FieldID), joinT.C(userrole.PermissionsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(userrole.PermissionsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(userrole.PermissionsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*RolePermission](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
