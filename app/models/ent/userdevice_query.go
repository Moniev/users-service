...
func (udq *UserDeviceQuery) QueryOwner() *UserQuery {
...
// QueryUserSettings2fa chains the current query on the "user_settings_2fa" edge.
func (udq *UserDeviceQuery) QueryUserSettings2fa() *UserSettingsQuery {
	query := (&UserSettingsClient{config: udq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := udq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QuerySecondFactorCodes chains the current query on the "second_factor_codes" edge.
func (udq *UserDeviceQuery) QuerySecondFactorCodes() *SecondFactorCodeQuery {
	query := (&SecondFactorCodeClient{config: udq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := udq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (udq *UserDeviceQuery) WithOwner(opts ...func(*UserQuery)) *UserDeviceQuery {
	query := (&UserClient{config: udq.config}).Query()
...
// WithUserSettings2fa tells the query-builder to eager-load the nodes that are connected to
// the "user_settings_2fa" edge. The optional arguments are used to configure the query builder of the edge.
func (udq *UserDeviceQuery) WithUserSettings2fa(opts ...func(*UserSettingsQuery)) *UserDeviceQuery {
	query := (&UserSettingsClient{config: udq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	udq.withUserSettings2fa = query
	return udq
}

// WithSecondFactorCodes tells the query-builder to eager-load the nodes that are connected to
// the "second_factor_codes" edge. The optional arguments are used to configure the query builder of the edge.
func (udq *UserDeviceQuery) WithSecondFactorCodes(opts ...func(*SecondFactorCodeQuery)) *UserDeviceQuery {
	query := (&SecondFactorCodeClient{config: udq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (udq *UserDeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserDevice, error) {
	...
	if query := udq.withOwner; query != nil {
		if err := udq.loadOwner(ctx, query, nodes, nil,
			func(n *UserDevice, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := udq.withUserSettings2fa; query != nil {
		if err := udq.loadUserSettings2fa(ctx, query, nodes, nil,
			func(n *UserDevice, e *UserSettings) { n.Edges.UserSettings2fa = e }); err != nil {
			return nil, err
		}
	}
...
func (udq *UserDeviceQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*UserDevice, init func(*UserDevice), assign func(*UserDevice, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*UserDevice)
	for i := range nodes {
		if nodes[i].user_user_devices == nil {
			continue
		}
		fk := *nodes[i].user_user_devices
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_user_devices" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (udq *UserDeviceQuery) loadUserSettings2fa(ctx context.Context, query *UserSettingsQuery, nodes []*UserDevice, init func(*UserDevice), assign func(*UserDevice, *UserSettings)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*UserDevice)
	for i := range nodes {
		if nodes[i].user_settings_second_factor_target == nil {
			continue
		}
		fk := *nodes[i].user_settings_second_factor_target
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(usersettings.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_settings_second_factor_target" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (udq *UserDeviceQuery) loadSecondFactorCodes(ctx context.Context, query *SecondFactorCodeQuery, nodes []*UserDevice, init func(*UserDevice), assign func(*UserDevice, *SecondFactorCode)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*UserDevice)
	for i := range nodes {
		if nodes[i].second_factor_code_target_user_device == nil {
			continue
		}
		fk := *nodes[i].second_factor_code_target_user_device
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(secondfactorcode.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "second_factor_code_target_user_device" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
