...
func (rpq *RolePermissionQuery) QueryUserRole() *UserRoleQuery {
	query := (&UserRoleClient{config: rpq.config}).Query()
...
func (rpq *RolePermissionQuery) Clone() *RolePermissionQuery {
	...
	return &RolePermissionQuery{
		...
		withUserRole: rpq.withUserRole.Clone(),
...
func (rpq *RolePermissionQuery) WithUserRole(opts ...func(*UserRoleQuery)) *RolePermissionQuery {
	query := (&UserRoleClient{config: rpq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rpq.withUserRole = query
	return rpq
}
...
//		GroupBy(rolepermission.FieldName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.RolePermission.Query().
//		Select(rolepermission.FieldName).
//		Scan(ctx, &v)
...
func (rpq *RolePermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RolePermission, error) {
	...
	if query := rpq.withUserRole; query != nil {
		if err := rpq.loadUserRole(ctx, query, nodes,
			func(n *RolePermission) { n.Edges.UserRole = []*UserRole{} },
			func(n *RolePermission, e *UserRole) { n.Edges.UserRole = append(n.Edges.UserRole, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rpq *RolePermissionQuery) loadUserRole(ctx context.Context, query *UserRoleQuery, nodes []*RolePermission, init func(*RolePermission), assign func(*RolePermission, *UserRole)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*RolePermission)
	nids := make(map[int]map[*RolePermission]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(rolepermission.UserRoleTable)
		s.Join(joinT).On(s.C(userrole.FieldID), joinT.C(rolepermission.UserRolePrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(rolepermission.UserRolePrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(rolepermission.UserRolePrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*RolePermission]struct{}{byID[outValue]: {}}
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "user_role" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
