...
func (lq *LocationQuery) QueryUserDetails() *UserDetailsQuery {
	query := (&UserDetailsClient{config: lq.config}).Query()
...
func (lq *LocationQuery) Clone() *LocationQuery {
	...
	return &LocationQuery{
		...
		withUserDetails: lq.withUserDetails.Clone(),
...
func (lq *LocationQuery) WithUserDetails(opts ...func(*UserDetailsQuery)) *LocationQuery {
	query := (&UserDetailsClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withUserDetails = query
	return lq
}
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	var (
		...
		_spec       = lq.querySpec()
		loadedTypes = [1]bool{
			lq.withUserDetails != nil,
		}
	)
	if lq.withUserDetails != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := lq.withUserDetails; query != nil {
		if err := lq.loadUserDetails(ctx, query, nodes, nil,
			func(n *Location, e *UserDetails) { n.Edges.UserDetails = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LocationQuery) loadUserDetails(ctx context.Context, query *UserDetailsQuery, nodes []*Location, init func(*Location), assign func(*Location, *UserDetails)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Location)
	for i := range nodes {
		if nodes[i].user_details_locations == nil {
			continue
		}
		fk := *nodes[i].user_details_locations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_details_locations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
