...
func (s *UsersService) UpdateUser(ctx context.Context, userID int, req *requests.User) (*ent.User, error) {
	...
	if updatedUser.Phone != user.Phone {
		if err := s.EventNotifier.CreateVerificationEvent(updatedUser.Edges.UserSettings, updatedUser.Phone, user.Edges.VerificationCode); err != nil {
	...
	if updatedUser.Mail != user.Mail {
		if err := s.EventNotifier.CreateRegistrationEvent(updatedUser, user.Edges.ActivationCode); err != nil {
...
func (s *UsersService) AddRole(ctx context.Context, userID, roleID int) (*ent.User, error) {
	return s.UsersRepository.AddRole(ctx, userID, roleID)
}

func (s *UsersService) RevokeRole(ctx context.Context, userID, roleID int) (*ent.User, error) {
